...
function buildSea(arg) {
  ...
  seaModules.forEach(name => {
    try {
      ...
      // Remove existing IIFE wrapper if present
      code = code.replace(/^\s*;?\s*\(\s*function\s*\(\s*\)\s*\{/, '');
      code = code.replace(/\}\s*\(\s*\)\s*\)?\s*;?\s*$/, '');
      
      // Remove existing require() calls and replace with USE()
      code = code.replace(/require\(['"]\.\/([\w-]+)['"]\)/g, "USE('./$1')");
      
      // Clean up whitespace
      code = code.trim();
      
      // Wrap in USE with proper indentation
